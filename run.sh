#!/bin/bash
#Run.sh
while [ ${#} -gt 0 ];do OPTERR=0;OPTIND=1;getopts "hdpr" arg;case "$arg" in
  h) help=true
  ;;
  d) development=true
  ;;
...
if [[ ! -z "$help" ]]; then
  ...
  printf "./run.sh [ -d -r ] | [-h]\n\n"
  printf " -d   Development mode. Starts with hot-reload and monts the volumes from host to container.\n"
  printf "      Production mode. Starts the node server without hot reload and without monting volumes.\n      Default mode if -d is not provided.\n"
